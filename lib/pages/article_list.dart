...
class _ArticleListState extends State<ArticleList> {
  ...
  void onReturnToPage(int lastArticleId) async {
    ...
    // Set<int> liServers = {};
    // List<UnreadId> dbUnreadIds = await databaseService.unreadIds();
    ...
    //   Server server = await databaseService.server(serverId);
    //   List<int> readIds = [];

    //   // var unreads = await databaseService.getArticlesNotInUnreadByServer(serverId);
    //   var unreadIds =
    //   readIds.addAll(unreads);
    //   await api.markIdsAsRead(server.baseUrl, server.auth, readIds);
    // }
    ...
    //   Server server = await databaseService.server(serverId);
    //   List<int> starredIds = [];
    //   var sIds = await databaseService.getArticlesToStarByServer(serverId);
    //   starredIds.addAll(sIds);
    //   await api.markIdsAsStarred(server.baseUrl, server.auth, starredIds);
    // }
    // await  api.markAsRead(widget.);
  ...
  Future<void> sortArticles() async {
    ...
    if (!isInitialBuild) {
      ...
      List<UnreadId> liUnreadId = await databaseService.unreadIds();
      ...
      List<StarredId> liStarredId = await databaseService.starredIds();
      ...
      for (int serverId in liIdsToMarkRead.keys) {
        Server server = await databaseService.server(serverId);
        if (serverId != 0) {
          await api.markIdsAsRead(server.baseUrl, server.auth, liIdsToMarkRead[serverId] ?? []);
        }
      }
      for (int serverId in liIdsToMarkStarred.keys) {
        Server server = await databaseService.server(serverId);
        if (serverId != 0) {
          await api.markIdsAsStarred(
              server.baseUrl, server.auth, liIdsToMarkStarred[serverId] ?? []);
        }
      }
      ...
      for (int serverId in liIdsToMarkUnRead.keys) {
        Server server = await databaseService.server(serverId);
        if (serverId != 0) {
          await api.markAsUnread(server.baseUrl, server.auth, liIdsToMarkUnRead[serverId] ?? []);
        }
      }
      for (int serverId in liIdsToMarkNotStarred.keys) {
        Server server = await databaseService.server(serverId);
        if (serverId != 0) {
          await api.markAsNotStarred(
              server.baseUrl, server.auth, liIdsToMarkNotStarred[serverId] ?? []);
        }
      }
